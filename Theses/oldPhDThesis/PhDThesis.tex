...
\documentclass[12pt, openright]{report} %% use instead when print-out is scaled down to B5

\usepackage[a4paper, margin=25mm]{geometry} %% use wider a4paper definition from geometry package
%\usepackage[b5paper, top=25mm, bottom=25mm]{geometry} %% use instead when print-out is in B5
...
\usepackage[hidelinks=true]{hyperref} %% generate links in the PDF (but no boxes around them)
\usepackage{longtable}        %% for tables then can go over several pages
\usepackage{pdfpages}         %% for inclusion of PDF documents
\usepackage{parskip}          %% no indent but line skip between paragrahps
\usepackage{sectsty}          %% for setting some headings
...
\begin{titlepage}
  ...
  \begin{center}
    \large{Thesis submitted to the University College of Southeast Norway\\ for the degree of philosophiae doctor~(PhD)}
  \end{center}
  \vspace*{\stretch{2}}
...
\includepdf[pages=1-5,openright]{figs/pdfpages}
